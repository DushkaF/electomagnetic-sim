...
class minimize():
    ...
    def __init__(self, func, x0, opt_mode, cooling_schedule='linear', step_max=1000, t_min=0, t_max=100, bounds=[],
                 alpha=None, damping=1):
        ...
        assert cooling_schedule in ['linear', 'exponential', 'logarithmic',
                                    'quadratic'], 'cooling_schedule must be either "linear", "exponential", "logarithmic", or "quadratic"'
        ...
        while self.step < self.step_max and self.t >= self.t_min and self.t > 0:
            ...
            # get neighbor
            proposed_neighbor = np.array(self.get_neighbor())
            ...
            print(
                "current E: {0:0.2f}, dE: {1:0.2f}, T: {2:0.2f}, probability: {3:0.2f}".format(self.current_energy, dE,
                                                                                               self.t, self.safe_exp(
                        -dE / self.t)))
